...
export interface DocumentPage {
  ...
  /**
   * Scan history (history design section 6). `true` for a page rehydrated from
   * IndexedDB, where `originalBlob` is the WARP BASE rather than the raw
   * capture — the history deliberately does not persist the original.
   *
   * Every consumer keeps working unchanged (the field is still a real,
   * decodable blob of the right dimensions); this flag exists so the UI can
   * tell the user that re-cropping a restored page cuts into the
   * already-straightened result instead of re-detecting the sheet.
   */
...
export interface DocumentSlice {
  /**
   * Stable identity for the document currently being built (history design
   * section 5). Minted once and regenerated by `resetDocument`, so writing to
   * the scan history is an idempotent `put`: exporting the same document twice
   * updates one record instead of creating two.
   */
...
export interface DocumentActions {
  ...
  /** Full teardown: close activeWorking, close all thumbnails, close pendingDeletion, reset to initial (mints a fresh `documentId`). */
  readonly resetDocument: () => void;

  // ── scan history (history design section 6) ─────────────────────
  /**
   * Replaces the entire document with pages rehydrated from the history, and
   * adopts the stored document's identity so a later re-export updates that
   * same record instead of forking a duplicate.
   *
   * Tears down the outgoing document exactly like `resetDocument` — restoring
   * over a live document must not leak its bitmaps. Lands in the `grid` phase,
   * which is the only phase that can render an arbitrary page set without a
   * capture or processing step behind it.
   */
  readonly loadDocumentFromHistory: (documentId: string, pages: readonly DocumentPage[]) => void;
}

export const initialDocumentSlice: DocumentSlice = {
  // Placeholder identity: `scannerStore.ts` seeds a real one at store creation
  // and `resetDocument` mints a fresh one per document. Left empty here so
  // `initialDocumentSlice` stays a pure constant that tests can compare
  // against without a random field defeating equality.
  documentId: '',
  pages: [],
  rawCaptures: [],
  activePageId: null,
...
/**
 * Full teardown (design section 1.5, extended Fase 2.3 for `rawCaptures`):
 * close `activeWorking`, every page's thumbnail, every raw capture's
 * thumbnail, and `pendingDeletion`'s thumbnail — never leave a live bitmap for
 * the GC to eventually reclaim.
 *
 * Shared by `resetDocument` and `loadDocumentFromHistory`, which both replace
 * the whole document wholesale and therefore owe the outgoing one the exact
 * same release.
 */
function releaseDocumentResources(state: DocumentSlice): void {
...
export function createDocumentActions(
  ...
  return {
    ...
    resetDocument: () =>
      set((state) => {
        releaseDocumentResources(state);
        // A fresh document gets a fresh identity, so the next history write
        // creates its own record instead of overwriting the one just finished
        // (history design section 5).
        return { ...initialDocumentSlice, documentId: randomId() };
      }),

    loadDocumentFromHistory: (documentId, pages) =>
      set((state) => {
        // Same teardown as `resetDocument` — restoring over a live document
        // must not leak the outgoing one's bitmaps.
        releaseDocumentResources(state);
        return {
