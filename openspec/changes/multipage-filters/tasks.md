...
- [x] 4.1 New `src/features/scanner/components/FilterPanel.tsx` (may use existing `Sheet`): 6 preset tiles previewed on the active page's thumbnail via `filterPipeline.needsWorker`/`buildCssFilter` routing (design section 3.4).
- [x] 4.2 Brightness/contrast/sharpness sliders mapped per design section 3.2/3.3; debounce worker calls at `FILTER.SLIDER_DEBOUNCE_MS` (~120ms).
- [x] 4.3 Batch the 3 adaptive-preset thumbnail previews in a single `APPLY_FILTER` call (spec scenario "Preview de los 6 presets sin recompute full-res").
- [x] 4.4 "Apply to all" button: confirmation step, then `applyFilterToAll(filter)` -- instant, no worker batch (spec scenario "Aplicar a todo el documento reescribe recetas sin renderizar").
- [x] 4.5 Wire `FilterPanel` into `CornerEditor`/active-page view; writes go through `updateRecipe(activePageId, withFilter(recipe, filter))`. Confirm filter changes never call `runWarp`. DESIGN DEVIATION (documented in `FilterPanel.tsx`/`CornerEditor.tsx` doc-comments): `FilterPanel` is composed INSIDE `CornerEditor` (design section 5.4 integration option (b)), not rendered by `ScannerScreen`, because `CornerEditor` is a controlled component that keeps ALL recipe edits (including rotate/flip, per its existing contract) LOCAL until Confirm -- filter edits follow the exact same non-destructive, discard-on-cancel pattern via `editRecipe.withFilter`. The commit to the store still happens through the EXISTING `updateRecipe`/`addPage` call sites (`useActivePage.rewarpActivePage`/`materializeCapture`), which already write the whole recipe (now including `filter`) unchanged -- so the design's "writes go through updateRecipe" intent is preserved at the commit boundary, just not as a direct per-keystroke store write. The ONE exception is "Apply to all" (ADR-011, inherently document-wide, not a per-session edit): `CornerEditor` accepts an `onApplyToAll` callback prop it only FORWARDS (never calls), wired to `DocumentSlice.applyFilterToAll` at the `ScannerScreen` level -- this is the sole store touchpoint reachable through `CornerEditor`'s tree, and it does not compromise `CornerEditor`'s own store-agnostic contract (a prop passthrough, not a store import).
- [x] 4.6 New `tests/unit/filterPanel.test.tsx`: preset selection calls `onChange` (the recipe-change handler) not warp (4 tests), slider debounce (rapid changes coalesce into one batched `APPLY_FILTER` call), apply-to-all confirmation writes instantly via `onApplyToAll` with zero worker calls, and the apply-to-all action is hidden when `onApplyToAll` is omitted.
- [x] 4.7 Verify: `npx vitest run filterPanel` (4/4 green); `npx tsc --noEmit` clean; `npx vitest run` full suite (24 files / 207 tests, all green, +1 file/+4 tests vs. Group 3's 23/203).
